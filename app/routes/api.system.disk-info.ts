...
const getDiskInfo = async (): Promise<DiskInfo[]> => {
  try {
    // Determine platform and try to import child_process only in Node environments
    const isNodeEnv = typeof process !== 'undefined' && Boolean(process.platform) && Boolean(process.versions?.node);
    let execSync: ((command: string, options?: { encoding?: BufferEncoding }) => string) | undefined;

    if (isNodeEnv) {
      try {
        const childProc = await import('node:child_process');
        execSync = childProc.execSync as unknown as typeof execSync;
      } catch (e) {
        // child_process not available in this runtime (e.g., Cloudflare Pages)
        execSync = undefined;
      }
    }

    // If execSync is not available, return an explicit error (no mock data)
    if (!execSync) {
      return [
        {
          filesystem: 'N/A',
